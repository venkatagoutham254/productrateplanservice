...
public class ProductAssembler implements SimpleRepresentationModelAssembler<ProductDTO> {
	...
	public void addLinks(final EntityModel<ProductDTO> entityModel) {
		...
		entityModel.add(linkTo(methodOn(ProductResource.class).getAllProducts(null, null, null, null, null)).withRel(IanaLinkRelations.COLLECTION));
	}
	@Override
	public void addLinks(final CollectionModel<EntityModel<ProductDTO>> collectionModel) {
		collectionModel.add(linkTo(methodOn(ProductResource.class).getAllProducts(null, null, null, null, null)).withSelfRel());
	}
